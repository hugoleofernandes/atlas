...
public sealed class ProcessOutboxCommandHandlerTests
{
    ...
    private static readonly ProcessOutboxCommand DefaultCommand =
        new(BatchSize: 10, MaxRetries: 3, LockDuration: TimeSpan.FromSeconds(30));

    public ProcessOutboxCommandHandlerTests()
    {
        _repository.AddRetryAsync(Arg.Any<OutboxMessage>(), Arg.Any<CancellationToken>())
            .Returns(Task.CompletedTask);
        _repository.AddExecutionsAsync(
    ...
    private void GivenBatch(params OutboxMessage[] messages) =>
        _repository.GetPendingBatchAsync(
    ...
    public async Task ExecuteAsync_WhenAllHandlersSucceed_ShouldNotAddRetry()
    {
        ...
        await _repository.DidNotReceive()
            .AddRetryAsync(Arg.Any<OutboxMessage>(), Arg.Any<CancellationToken>());
    }
    ...
    public async Task ExecuteAsync_WhenHandlerFailsOnNonFinalAttempt_ShouldRetryHaveIncrementedAttemptNumber()
    {
        ...
        _repository.AddRetryAsync(
            Arg.Do<OutboxMessage>(m => capturedRetry = m),
            Arg.Any<CancellationToken>())
            .Returns(Task.CompletedTask);
    ...
    public async Task ExecuteAsync_WhenHandlerFailsOnNonFinalAttempt_ShouldRetryShareIdempotencyKey()
    {
        ...
        // Assert
        capturedRetry!.IdempotencyKey.Should().Be(message.IdempotencyKey,
            "idempotency key must be stable across the entire attempt chain");
    ...
    public async Task ExecuteAsync_WhenHandlerFailsOnFinalAttempt_ShouldStillAddExecutionRecords()
    {
        ...
        await _repository.Received(1)
            .AddExecutionsAsync(
                Arg.Is<IReadOnlyList<OutboxHandlerExecution>>(e =>
                    e.Count == 1 && e[0].Status == "Failure"),
                Arg.Any<CancellationToken>());
    }
