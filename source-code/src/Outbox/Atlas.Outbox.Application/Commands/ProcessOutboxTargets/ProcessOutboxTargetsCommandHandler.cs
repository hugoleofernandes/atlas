    ...
    : IProcessOutboxTargetsCommandHandler
{
        ...
        CancellationToken ct)
    {
        ...
        var executorMap = executors.ToDictionary(executor => executor.Mode);

        var results = new List<HandlerInvocationResult>(command.Targets.Count);

        foreach (var target in command.Targets)
        {
            if (!executorMap.TryGetValue(target.Mode, out var executor))
            {
            ...
            results.Add(await executor.ExecuteAsync(target, message, ct));
        }
