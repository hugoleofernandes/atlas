...
public sealed class ProcessOutboxCommandHandler : IIdentityOutboxCommandHandler, IStaffOutboxCommandHandler
{
    ...
    {
        var messages = await _repository.GetPendingBatchAsync(command.BatchSize, command.LockDuration, ct);
        ...
        foreach (var message in messages)
        {
            ...
            try
            {
                ...
                {
                    var errorSummary =
                        $"{failures.Count} of {results.Count} handler(s) failed on attempt {message.AttemptNumber}.";
    ...
    {
        return results
            .Select(r => new OutboxHandlerExecution(outboxMessageId, r.HandlerName, r.IsSuccess, r.ErrorMessage))
            .ToList();
    }
}
