...
public static class OutboxInfrastructureServiceCollectionExtensions
{
    ...
    {
        services.AddSingleton<IIntegrationEventTypeResolver>(_ => new IntegrationEventTypeResolver(
            integrationEventAssemblies
        ));
        ...
        services.AddScoped<IOutboxTargetExecutor, DirectTargetExecutor>();
        services.AddScoped<IOutboxTargetExecutor, HttpTargetExecutor>();

        var workerOptions =
            configuration.GetSection("OutboxWorker").Get<OutboxWorkerOptions>() ?? new OutboxWorkerOptions();

        if (string.Equals(workerOptions.DispatchMode, "Http", StringComparison.OrdinalIgnoreCase))
            services.AddSingleton<ITargetCatalog, HttpOutboxTargetCatalog>();

        services.AddScoped<ITargetCatalogReader, OutboxTargetResolver>();
