...
[Queue(HangfireOutboxQueues.Identity)]
public sealed class ProcessIdentityOutboxHangfireJob(
    IIdentityOutboxProcessingWorkflow workflow,
    IOptions<OutboxWorkerOptions> workerOptions,
    IOptions<HangfireOutboxOptions> hangfireOptions,
    ILogger<ProcessIdentityOutboxHangfireJob> logger)
{
    ...
    public async Task ExecuteAsync(IJobCancellationToken cancellationToken)
    {
        ...
        var command = new ProcessOutboxCommand(worker.BatchSize, worker.MaxRetries, worker.LockDuration, "identity");
        var startedAt = DateTime.UtcNow;

        using (logger.BeginScope(new Dictionary<string, object> { ["Module"] = "identity" }))
        {
            logger.LogInformation(
                "HangfireOutbox: identity job started (window={Window}s, poll={Poll}s)",
            ...
            logger.LogInformation(
                "HangfireOutbox: identity job completed in {Elapsed:F1}s",
