...
public sealed class IdentityListDeadLettersReader(IdentityDbContext db) : IListDeadLettersReader
{
    private const string Sql = """
        SELECT
            ...
            m.error,
            EXISTS (
                SELECT 1 FROM atlas_identity.outbox_messages c
                WHERE  c.parent_outbox_message_id = m.id
            ) AS WasResubmitted
        FROM   atlas_identity.outbox_messages m
