...
/// </summary>
public abstract class GetPendingMessagesReader : IGetPendingMessagesReader
{
    ...
    protected GetPendingMessagesReader(DbContext db, string schema)
    {
        _db     = db;
        _schema = schema;
    }
    ...
    public async Task<IReadOnlyList<OutboxMessageDto>> ReadAsync(
        int batchSize,
        ...
        CancellationToken ct)
    {
        ...
        var results = await conn.QueryAsync<OutboxMessageDto>(
            new CommandDefinition(
                FetchSql,
                new { BatchLockId = batchLockId },
                cancellationToken: ct));
