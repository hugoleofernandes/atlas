...
) : IIdentityOutboxProcessingWorkflow, IStaffOutboxProcessingWorkflow
{
    ...
    {
        // Step 1 - Fetch pending messages via query handler.
        var getPendingMessagesQuery = new GetPendingMessagesQuery(command.BatchSize, command.LockDuration);
        ...
        foreach (var message in messages)
        {
            ...
            if (!resolveTargetsResult.IsSuccess || resolveTargetsResult.Value is null)
            {
            ...
            else
            {
                ...
                if (!processTargetsResult.IsSuccess || processTargetsResult.Value is null)
                {
                    var errorMessage =
    ...
    private void LogLockedMessage(OutboxMessageDto message)
    {
    ...
    private void LogResolvedTargets(OutboxMessageDto message, IReadOnlyList<OutboxDispatchTargetDto> targets)
    {
        logger.LogInformation(
    ...
    private void LogFinalizationResult(OutboxMessageDto message, Result<UpdateOutboxMessageStatusOutput> updateResult)
    {
